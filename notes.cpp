...
int main() {
    ...
    // The variable name is case sensitive, which means "A" is differnt from "A"
    ...
    // Overflow error may occur without warning when exceed max or is below max value

    float floating_point_number = 3.9; // real number (4 bytes / 32 bits)
    double double_number = 3e-2; // default real number (8 bytes / 64 bits)

    // Character
    char character = 'd'; // single character of ASCII code (MUST use '', 1 bytes / 8 bits)
    cout << character + 1 << endl; // 100 (ASCII of 'd') + 1 = 101
    character = character + 1;
    cout << character << endl; // 'e'
    // '\' is a escape character, which can help writing special character like tab or "
    ...
    // with namespace used (in global scope), variables inside can be accessed outside,
    // like this: namespace level 1 { int x = 10; } 


    // sizeof() operator
    // this will return the memory space used of a variable or a type of variable
